...
export function VideoCard({ title, videoId, thumbnailUrl, date, duration, category }: VideoCardProps) {
    ...
    // Use i.ytimg.com (more reliable), start with maxresdefault, fall back through alternatives
    const fallbackUrls = [
        thumbnailUrl,
    ...
    const currentSrc = fallbackUrls[imgIndex] || fallbackUrls[0]

    const handleError = () => {
        if (imgIndex < fallbackUrls.length - 1) {
